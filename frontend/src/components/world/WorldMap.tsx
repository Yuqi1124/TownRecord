...
import Player, { ServerPlayer, ServerPlayerPermissions, UserLocation } from '../../classes/Player';
...
type ConversationGameObjects = {
  ...
  limitText: Phaser.GameObjects.Text;
...
class CoveyGameScene extends Phaser.Scene {
  ...
  private requestTextBox?: Phaser.GameObjects.Text;

  private setRequestOpen: (open: boolean) => void;
  ...
  constructor(
    ...
    this.setRequestOpen = setRequestOpen;
  }
  ...
  getConversation(): ServerConversationArea | undefined {
    return this.currentConversationArea?.conversationArea?.toServerConversationArea();
  }

  getConversationOrigin(): ConversationArea | undefined {
    ...
    conversationAreas.forEach(eachNewArea => {
      ...
      if (existingArea) {
        ...
        const updateListener = {
          ...
          onStatusChange: (newStatus: ConversationAreaStatus) => {
            // this.setNewStatus(newStatus);
            switch (newStatus) {
              case ConversationAreaStatus.AvailableToRequest:
                existingArea.statusText.text = 'Available';
                break;
              case ConversationAreaStatus.DoNotDisturb:
                existingArea.statusText.text = 'Do Not Disturb';
                break;
              case ConversationAreaStatus.Public:
                existingArea.statusText.text = 'Public';
                break;
              default:
                break;
            }
          ...
          onLimitChange: (newLimit: string | undefined) => {
            if (newLimit) {
              existingArea.limitText.text = newLimit;
            } else {
              existingArea.limitText.text = '';
            }
          },
        ...
        updateListener.onStatusChange(eachNewArea.status);
        // updateListener.onLimitChange(eachNewArea.limit)
      }
    });
  ...
  updatePlayerLocation(player: Player) {
    ...
    if (!myPlayer) {
      ...
      myPlayer = new Player(player.id, player.userName, location, player.permissions);
      ...
      if (
        ...
        if (this.currentConversationArea) {
          ...
          if (
            ...
            this.requestTextBox?.setVisible(false);
  ...
  create() {
    ...
    conversationSprites.forEach(conversation => {
      ...
      const limitText = this.add.text(
        sprite.x + sprite.displayWidth / 2,
        15 + sprite.y - sprite.displayHeight / 2,
        '',
        { color: '#000000' },
      );
      sprite.setTintFill();
    ...
    this.requestTextBox = this.add
      .text(
        this.game.scale.width / 2,
        this.game.scale.height / 2,
        "You've found a private Conversation Area!\n If you would like to join,\n press the spacebar.",
        { color: '#000000', backgroundColor: '#FFFFFF' },
      )
      .setScrollFactor(0)
      .setDepth(30);
    this.requestTextBox.setVisible(false);
    this.requestTextBox.x = this.game.scale.width / 2 - this.requestTextBox.width / 2;
    ...
    this.physics.add.overlap(
      ...
      (overlappingPlayer, conversationSprite) => {
        ...
        if (conv?.conversationArea) {
          this.infoTextBox?.setVisible(false);
          this.requestTextBox?.setVisible(false);
          if (conv.conversationArea.status === ConversationAreaStatus.AvailableToRequest) {
            if (cursorKeys.space.isDown) {
              this.setRequestOpen(true);
            }
            this.requestTextBox?.setVisible(true);
          }
        } else {
          if (cursorKeys.space.isDown) {
...
export default function WorldMap(): JSX.Element {
  ...
  const [currentConversation, setCurrentConversation] = useState<ConversationArea>();
  ...
  useEffect(() => {
    ...
    setCurrentConversation(gameScene?.getConversationOrigin())
  ...
  useEffect(() => {
    const conversation = gameScene?.getConversation();
    if (conversation && conversation?.status === ConversationAreaStatus.AvailableToRequest) {
      video?.pauseGame();
    } else {
      video?.unPauseGame();
    }
  ...
  const adminSidebar = useMemo(() => {
    const currentPlayer = players.find(pl => pl.id === myPlayerID);
    if (currentPlayer?.permissions === ServerPlayerPermissions.Admin) {
      return (
          <div id='admin-container'>
            <AdminSidebar currentConversationArea={currentConversation} />
          </div>
        );
     }
    return (<></>);
  }, [players, myPlayerID, currentConversation])

  const jrequest : JoinRequest[] = [];
  const joinRequestsRecord = useRef(jrequest);

  const toast = useToast();

  useEffect(() => {
    const currentCA = conversationAreas.find(area => area.label === currentConversation?.label);
    if (currentCA?.joinRequests !== joinRequestsRecord.current) {
      for(let i = 0; i < joinRequestsRecord.current.length; i += 1){
        if(currentCA === undefined || !currentCA.joinRequests.includes(joinRequestsRecord.current[i])) {
          if (myPlayerID === joinRequestsRecord.current[i].playerId) {
            toast({
              title: 'Join Request Been Decided!',
              status: 'success',
          });
          }
        }
    ...
    joinRequestsRecord.current = Object.assign([], currentCA?.joinRequests);
  }, [conversationAreas, joinRequestsRecord])

  const joinRequestModal = useMemo(() => {
    const conversation = gameScene?.getConversation();
    if (requestOpen && conversation) {
      video?.pauseGame();
      return (
        <JoinRequestModal
    ...
    return <></>;
  }, [video, gameScene, requestOpen, setRequestOpen]);
