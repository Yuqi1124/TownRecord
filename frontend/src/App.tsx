...
import './App.css';
import ConversationArea, { ConversationAreaStatus, ServerConversationArea } from './classes/ConversationArea';
  ...
  const setupGameController = useCallback(
    async (initData: TownJoinResponse) => {
      ...
      socket.on('PlayerStatusChanged', (player: ServerPlayer) => {
        ...
        if (updatePlayer) {
          const clonePlayers = [...localPlayers];
          clonePlayers.forEach(
              (pl) => {
                if (pl.id === player._id) {
                  pl.permissions = player._permissions;
                }
              }
          );
          localPlayers = clonePlayers;
          setPlayersInTown(clonePlayers);
        }
        else {
      ...
      socket.on('conversationUpdated', (_conversationArea: ServerConversationArea) => {
        ...
        if (updatedConversationArea) {
          const cloneAreas = [...localConversationAreas];
          cloneAreas.forEach(
            (ca) => {
              if (ca.label === _conversationArea.label) {
                ca.topic = _conversationArea.topic;
                ca.occupants = _conversationArea.occupantsByID;
                ca.status = _conversationArea.status;
                ca.maxOccupants = _conversationArea.maxOccupants;
                ca.joinRequests = _conversationArea.joinRequests;
                }
              }
          );
          localConversationAreas = cloneAreas;
          setConversationAreas(cloneAreas);
        } else {
          const newCA = ConversationArea.fromServerConversationArea(_conversationArea);
      ...
      socket.on('conversationDestroyed', (_conversationArea: ServerConversationArea) => {
        ...
        if(existingArea){
          ...
          existingArea.status = ConversationAreaStatus.Public;
